...
CHESSAPP.ui = (function(){
	...
	var createVoteList = function(){
		var voteListContainer = document.createElement("div"),
		voteListScroll = document.createElement("div"),
		header = document.createElement("h2");
		voteList = document.createElement("div");
		header.appendChild(document.createTextNode("votes"));
		voteListContainer.className = "movelist";
		voteListScroll.className = "scroll";
		voteListScroll.appendChild(voteList);
		voteListContainer.appendChild(header);
		voteListContainer.appendChild(voteListScroll);
		rightCol.appendChild(voteListContainer);
	};
	...
	var createProgbar = function(statusID){
		progbarDiv = document.createElement("div"),
	    progbarDiv.classList.add("progbar")
		progbar = document.createElement("span"),
		progbar.id = "voteProgbar"
		progbar.style.width = "0%"
		progbarDiv.appendChild(progbar);
		container.appendChild(progbarDiv);
	};
 ...
 */
	that.init = function(stg){
		...
		if(!elementsCreated){
			...
			CHESSAPP.ui.promotionActive = false
		}
	...
	that.voteStarted = function() {
		...
		var interval = window.setInterval(function() {
		  	...
		  	progbar.style.width = currentPercent + "%";
		}, 50);
	}
	...
	that.updateVotelist = function(table){
		var html = ""
		if (table == null) {
			voteList.innerHTML = ""
		} else {
			votes.forEach(function(item, index){
				html += item.message + " " + item.count + "\n"
			})
   ...
   */
	...
	that.setSelectionVisible = function(stg){
		...
		if(val){
			...
			CHESSAPP.ui.promotionActive = true
			CHESSAPP.ui.promotion_data = stg;
			toggleOverlay(true, "selection");
		}
		else{
			CHESSAPP.ui.promotionActive = false
			CHESSAPP.ui.romotion_data = null;
